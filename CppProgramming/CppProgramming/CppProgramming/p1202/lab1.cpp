...
public:
	...
	bool isAlive;		// 생존 여부
	bool isLoaded = false;		// DropShip 등에 Load 되었는가?
...
public:
	...
	/*
	 * Siege Mode 로 공격을 합니다.
	 * @param target	공격을 받을 대상
	 */
	virtual void siegeModeAttack(BaseUnit& target) {};

	/*
	 * 평범한 공격을 합니다.
	 */
	virtual void attack(BaseUnit& target) {
		int dmg = this->atkDamage - target.defDamage;
		if (dmg < 0) dmg = 0;
...
public:
	...
	void siegeModeAttack(BaseUnit& target) override {
		int dmg = this->dmgForSiegeMode - target.defDamage;
		if (dmg < 0) dmg = 0;
...
public:
	...
	void buy(BaseUnit* unit, int& currentMineral, int& currentGas) {}
};

template<>
class Building<Protoss> {
public:
	Building() {
		cout << "protoss building Construction completed." << endl;
	}

	void buy(BaseUnit* unit, int& currentMineral, int& currentGas) {
		unit->buy(currentMineral, currentGas);
	}
};
...
public:
	Building() {
		cout << "terran building Construction completed." << endl;
	}
...
template<typename T>
void multipleAttack(void (BaseUnit::* atkFunPtr)(BaseUnit& target), BaseUnit& target, T var1) {

}

template<typename T, typename... Args>
void multipleAttack(void (BaseUnit::* atkFunPtr)(BaseUnit& target), BaseUnit& target, T var1, Args... var2) {
	(var1.*atkFunPtr)(target);
	multipleAttack(atkFunPtr, target, var2...);
}


int main() {
	void (BaseUnit::*atkFunPtr)(BaseUnit & target);

	int mineral = 1000;
	int gas = 1000;

	Dragoon dragon;
	dragon.buy(mineral, gas);
	...
	atkFunPtr = &BaseUnit::attack;

	multipleAttack(atkFunPtr, dragon, tank, zealot);
